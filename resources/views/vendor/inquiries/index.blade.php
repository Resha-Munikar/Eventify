...
@section('title', 'Vendor - Inquiries')
@php 
$noNavbar = true;
$noFooter = true; 
@endphp
...
<div class="ml-0 sm:ml-64 p-3 sm:p-6 min-h-screen" x-data="vendorInquiryManager()">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-4 sm:p-6 border border-gray-100 dark:border-gray-700">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
            <div>
                <h2 class="text-2xl sm:text-3xl font-bold text-[#8d85ec]">Inquiries</h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Manage customer inquiries and questions for your events and venues.</p>
            </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
            ...
            <div class="p-4 rounded-xl bg-gray-50 dark:bg-gray-700/40 border border-gray-200 dark:border-gray-700 flex items-center gap-3">
                ...
                <div>
                    <div class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total Received</div>
            ...
            <div class="p-4 rounded-xl bg-amber-50/60 dark:bg-amber-900/20 border border-amber-200/80 dark:border-amber-700/50 flex items-center gap-3">
                ...
                <div>
                    <div class="text-[11px] font-bold text-amber-700 dark:text-amber-400 uppercase tracking-wider">Unread</div>
            ...
            <div class="p-4 rounded-xl bg-purple-50/60 dark:bg-purple-900/20 border border-purple-200/80 dark:border-purple-700/50 flex items-center gap-3">
                ...
                <div>
                    <div class="text-[11px] font-bold text-[#8d85ec] dark:text-purple-300 uppercase tracking-wider">Today</div>
        ...
        <form method="GET" action="{{ route('vendor.inquiries.index') }}" 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                    ...
                    <select id="type" 
                        ...
                        <option value="vendor" {{ request('type') === 'vendor' ? 'selected' : '' }}>General Vendor Inquiry</option>
                    </select>
                </div>
            ...
            <table class="w-full text-xs sm:text-sm text-left border-collapse">
                <thead class="bg-[#8D85EC] text-white text-[11px] sm:text-xs uppercase tracking-wider">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3">Related Event / Venue</th>
                        ...
                        <tr class="hover:bg-purple-50/50 dark:hover:bg-gray-700/50 transition {{ $inquiry->status === 'unread' ? 'bg-amber-50/30 dark:bg-amber-900/10 font-medium' : '' }}">
                            ...
                            <td class="px-4 py-3">
                                <div class="font-bold text-gray-900 dark:text-white flex items-center gap-1.5">
                                    ...
                                    @if($inquiry->status === 'unread')
                                        <span class="w-2 h-2 rounded-full bg-amber-500 inline-block" title="New Inquiry"></span>
                                    @endif
                                </div>
                            ...
                            <td class="px-3 py-3 text-center whitespace-nowrap">
                                @if($inquiry->type === 'event')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300">
                                        Event
                                    </span>
                                @elseif($inquiry->type === 'venue')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300">
                                        Venue
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-300">
                                        Vendor
                                    </span>
                                @endif
                            </td>
                            ...
                            <!-- Related Event/Venue -->
                            <td class="px-4 py-3">
                                <div class="font-semibold text-gray-900 dark:text-white truncate max-w-[220px]">
                                    @if($inquiry->event)
                                        {{ $inquiry->event->event_name }}
                                    @elseif($inquiry->venue)
                                        {{ $inquiry->venue->venue_name }}
                                    @else
                                        {{ $inquiry->subject ?? 'Direct Inquiry' }}
                                    @endif
                                </div>
                                <div class="text-[11px] text-gray-500 dark:text-gray-400 truncate max-w-[220px]">
                                    {{ \Illuminate\Support\Str::limit($inquiry->message, 45) }}
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3 text-center whitespace-nowrap">
                                <button type="button"
                                        ...
                                        class="inline-flex items-center gap-1 bg-[#8D85EC] hover:bg-[#7b76e4] text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition shadow-sm">
                                    <svg class="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center text-gray-500 dark:text-gray-400 font-medium">
                                ...
                                <p class="text-base font-semibold text-gray-700 dark:text-gray-300">No inquiries received yet.</p>
                                <p class="text-xs text-gray-400 mt-1">Customer inquiries submitted for your events and venues will appear here.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div @click.away="closeModal()" 
            ...
            <div class="flex items-start justify-between pb-4 border-b border-gray-100 dark:border-gray-700">
                <div>
                    ...
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white" x-text="activeInquiry?.subject || 'Customer Inquiry'"></h3>
                </div>
            ...
            <div class="py-4 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3.5 rounded-xl bg-gray-50 dark:bg-gray-700/40 border border-gray-200 dark:border-gray-600">
                    ...
                    <div>
                        <div class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase">Customer Email</div>
                    ...
                    <div x-show="activeInquiry?.phone">
                        <div class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase">Customer Phone</div>
                        <div class="text-sm font-semibold text-gray-900 dark:text-white" x-text="activeInquiry?.phone"></div>
                    </div>
                    <div>
                        <div class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase">Received At</div>
                ...
                <!-- Related Event / Venue -->
                <div x-show="activeInquiry?.event || activeInquiry?.venue" 
                     class="p-3.5 rounded-xl bg-purple-50/50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800/40 text-xs">
                    <div class="font-bold text-[#8d85ec] uppercase mb-1">Inquiry Context</div>
                    <template x-if="activeInquiry?.event">
                        <div class="text-gray-700 dark:text-gray-300">
                            <strong>Your Event:</strong> <span x-text="activeInquiry.event.event_name"></span> (<span x-text="activeInquiry.event.venue"></span>)
                        </div>
                    </template>
                    <template x-if="activeInquiry?.venue">
                        <div class="text-gray-700 dark:text-gray-300">
                            <strong>Your Venue:</strong> <span x-text="activeInquiry.venue.venue_name"></span> (<span x-text="activeInquiry.venue.location"></span>)
                        </div>
                    </template>
                </div>
                ...
                <!-- Message Body -->
                <div>
                    <div class="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-2">Customer Message</div>
...
function vendorInquiryManager() {
    return {
        ...
        changeStatus(newStatus) {
            ...
            fetch(`/vendor/inquiries/${inquiryId}/status`, {
                method: 'PATCH',
                headers: {
